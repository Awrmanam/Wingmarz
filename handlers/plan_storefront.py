...
async def _sellable_plans() -> list[Any]:
    """Return active paid plans whose Rebecca mapping contains an active service.

    Rebecca services are provider-side routing/inbound metadata. They are kept
    entirely internal for paid purchases; customers choose commercial plans.
    """
...
async def _render_plan_rows(
    ...
    *,
    ...
    if duration_label:
        back = f"planmarket:root:{source}"
    else:
        back = _home_callback(source)
