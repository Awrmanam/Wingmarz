...
# اختیاری؛ اگر خالی باشد برای هر بکاپ یک رمز قوی تازه ساخته می‌شود
BACKUP_ZIP_PASSWORD=
# اختیاری؛ در صورت خالی بودن، بکاپ برای همه SUDO_ADMINS ارسال می‌شود
BACKUP_RECIPIENTS=
EOF
```
...
- `.env`:
  ...
  - `BACKUP_AUTO_ENABLED`: فعال بودن بکاپ خودکار؛ پیش‌فرض `true`
  - `BACKUP_INTERVAL_HOURS`: فاصله بکاپ‌ها بر حسب ساعت؛ پیش‌فرض `1`
  - `BACKUP_RETENTION_COUNT`: تعداد نسخه‌های محلی نگهداری‌شده؛ پیش‌فرض `24`
  - `BACKUP_ZIP_PASSWORD`: رمز ثابت اختیاری؛ اگر خالی باشد هر فایل رمز تصادفی مستقل دارد
  - `BACKUP_RECIPIENTS`: آیدی تلگرام گیرنده‌های بکاپ؛ اگر خالی باشد از `SUDO_ADMINS` استفاده می‌شود

## بکاپ امن و خودکار
- ربات به‌طور پیش‌فرض هر ساعت از SQLite یک snapshot سازگار می‌گیرد و پیش از بسته‌بندی سلامت آن را بررسی می‌کند.
- آرشیو با AES-256 رمزگذاری می‌شود و شامل دیتابیس، تنظیمات زمان اجرا، فایل‌های استقرار، داده‌های جانبی و لاگ‌هاست.
- فایل فقط برای `BACKUP_RECIPIENTS` ارسال می‌شود. رمز ZIP، وضعیت رمز دیتابیس و دسترسی‌های لازم در پیام پاسخ مستقیم زیر فایل می‌آید.
- از منوی سودو می‌توان بکاپ دستی گرفت، فاصله خودکار را با `1h` تا `168h` تغییر داد یا آن را با `off` خاموش کرد.
- ریستور بکاپ‌های جدید رمز را می‌پرسد، سلامت SQLite را بررسی می‌کند و دیتابیس قبلی را با پسوند `.bak` نگه می‌دارد.
